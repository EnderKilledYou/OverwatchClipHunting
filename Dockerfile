...
ENV APP_HOME /app
ENV INSTALL_SCRIPT /app/install/install_tessy.sh
#Download Ocr Shit
...
WORKDIR $APP_HOME
COPY . ./
RUN chmod +x /app/install/post_install.sh
RUN chmod +x /app/install/install_tessy.sh
RUN /app/install/post_install.sh
